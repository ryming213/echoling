        ...
        Column(
            ...
            Column(
                ...
                val sections = listOf(
                    ...
                    "三、自动生成字幕" to listOf(
                        // (2026-07-18) 新功能：导入音视频素材时如果不选
                        // 字幕文件, ImportScreen 会显示「自动生成字幕」
                        // 卡片, 由 Vosk 在本地离线识别 SRT, 全程无需
                        // 联网、无需任何 API Key。
                        "导入音视频素材时如果**不选字幕文件**, ImportScreen 会出现「自动生成字幕」卡片——由 Vosk 本地离线识别 SRT, 全程不上传音频、无需联网、无需 API Key。",
                        "「立即转字幕」：导入时同步识别。卡片底部紫色进度条按 5% 实时推进, 完成后自动返回素材列表。",
                        "「稍后转字幕」：先返回素材列表。素材卡片上显示「字幕待识别」芯片, **点击即可开始识别**; 识别过程中卡片顶部出现紫色进度条, 每 5% 推进一次, 完成后自动消失。",
                        "识别失败的素材会显示「字幕识别失败, 点击重试」红色芯片; 点击即可清掉旧的 SRT 重新入队。",
                        "进入跟读练习时如果字幕还在识别, 会先停在「字幕正在识别中…」占位页, 识别完成后自动加载 SRT 进入正常练习界面, 无需手动刷新。",
                    ),
                    "四、跟读练习" to listOf(
